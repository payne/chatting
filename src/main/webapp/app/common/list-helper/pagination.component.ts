...
export class PaginationComponent implements OnChanges, OnInit {
  ...
  initViewData() {
    ...
    previous.params = this.getStepParams(Math.max(0, page.page - 1));
    this.steps.push(previous);
    // find a range of up to 5 pages around the current active page
    const startAt = Math.max(0, Math.min(page.page - 2, page.totalPages - 5));
    const endAt = Math.min(startAt + 5, page.totalPages);
    for (let i = startAt; i < endAt; i++) {
      const step = new PaginationStep();
      step.active = i == page.page;
      step.label = '' + (i + 1);
      step.params = this.getStepParams(i);
      this.steps.push(step);
    }
    const next = new PaginationStep();
    next.disabled = page.page > page.totalPages - 2;
    next.label = this.getMessage('next');
    next.params = this.getStepParams(Math.min(page.totalPages - 1, page.page + 1));
    this.steps.push(next);

    const rangeStart = page.page * page.size + 1;
...
export interface Page<PageType> {
